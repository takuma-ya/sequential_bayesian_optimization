...
void MainWidget::paintEvent(QPaintEvent *event)
{
    ...
    for (int pix_x = 0; pix_x < w; ++ pix_x)
    {
        for (int pix_y = 0; pix_y < h; ++ pix_y)
        {
            ...
            case Content::ExpectedImprovement:
                val(pix_x, pix_y) = (core.regressor.get() != nullptr) ? GPUCB::calculateGPUCB(*core.regressor, x) : 0.0;
                break;
            default:
                val(pix_x, pix_y) = 0.0;
                break;
  ...
  //  }
    ...
    if (draw_slider_tick)
    {
        ...
        //painter.drawEllipse(QPointF(0.4 * w, 0.6 * h), knob_back_radius, knob_back_radius);
        painter.setPen(sliderPen);
        painter.setBrush(sliderBrush);
        painter.drawEllipse(QPointF(x(0) * w, x(1) * h), knob_radius, knob_radius);
        //painter.drawEllipse(QPointF(0.4 * w, 0.6 * h), knob_radius, knob_radius);
    }
