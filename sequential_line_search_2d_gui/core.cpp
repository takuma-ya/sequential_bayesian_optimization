...
void Core::proceedOptimization()
{
    ...
    addData(std::vector<VectorXd>{ x, slider->X.row(0), slider->X.row(99), slider -> X.row(50) });
...
#else
        Eigen::MatrixXd xm =Eigen::MatrixXd::Zero(100, 2);
        for(int i; i < 100; i++){
            xm.row(i) = VectorXd::Constant(2,i*0.01); 
        }
        slider = std::make_shared<Slider>(xm, false);
#endif
        return;
    }

    const MatrixXd Xm = GPUCB::findNextPoint(*regressor);

    slider = std::make_shared<Slider>(Xm, true);
}
